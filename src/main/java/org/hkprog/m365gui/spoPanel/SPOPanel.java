...
public class SPOPanel extends javax.swing.JPanel {
	...
	private void showCreateSiteDialog() {
		...
		dialog.setTitle("Create SharePoint Site");
		...
		formPanel.add(new JLabel("Site Title:"), gbc);
		...
		formPanel.add(new JLabel("Site URL:"), gbc);
		gbc.gridx = 1; gbc.fill = GridBagConstraints.HORIZONTAL; gbc.weightx = 1.0;
		...
		formPanel.add(new JLabel("Site Type:"), gbc);
		gbc.gridx = 1; gbc.fill = GridBagConstraints.HORIZONTAL; gbc.weightx = 1.0;
		JComboBox<String> typeCombo = new JComboBox<>(new String[]{"TeamSite", "CommunicationSite"});
		formPanel.add(typeCombo, gbc);
		
		// Template (only for Communication Sites)
		gbc.gridx = 0; gbc.gridy = 3; gbc.fill = GridBagConstraints.NONE; gbc.weightx = 0;
		JLabel templateLabel = new JLabel("Template:");
		formPanel.add(templateLabel, gbc);
		gbc.gridx = 1; gbc.fill = GridBagConstraints.HORIZONTAL; gbc.weightx = 1.0;
		JComboBox<String> templateCombo = new JComboBox<>(new String[]{"Topic", "Showcase", "Blank"});
		...
		// Owner (for Team Sites)
		gbc.gridx = 0; gbc.gridy = 5; gbc.fill = GridBagConstraints.NONE; gbc.weightx = 0;
		JLabel ownerLabel = new JLabel("Owner Email:");
		formPanel.add(ownerLabel, gbc);
		gbc.gridx = 1; gbc.fill = GridBagConstraints.HORIZONTAL; gbc.weightx = 1.0;
		JTextField ownerField = new JTextField(20);
		formPanel.add(ownerField, gbc);
		
		// Enable/disable fields based on site type
		typeCombo.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				boolean isCommunication = "CommunicationSite".equals(typeCombo.getSelectedItem());
				templateCombo.setEnabled(isCommunication);
				templateLabel.setEnabled(isCommunication);
				ownerField.setEnabled(!isCommunication);
				ownerLabel.setEnabled(!isCommunication);
			}
		});
		...
		createButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// Validation
				if (title.isEmpty()) {
					JOptionPane.showMessageDialog(dialog, "Site title is required!", "Validation Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				if (url.isEmpty()) {
					JOptionPane.showMessageDialog(dialog, "Site URL is required!", "Validation Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				if ("TeamSite".equals(type) && owner.isEmpty()) {
					JOptionPane.showMessageDialog(dialog, "Owner email is required for Team Sites!", "Validation Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				
				dialog.dispose();
				createSiteInBackground(title, url, type, template, description, owner);
			}
		});
	 ...
	 */
	private void createSiteInBackground(String title, String url, String type, String template, String description, String owner) {
		SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
			@Override
			protected Void doInBackground() throws Exception {
				try {
